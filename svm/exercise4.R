#This example uses a somewhat complex nonlinear decision boundary,
# the same as example 3, but uses nu-SVM regularization.
...
# The parameter nu is an upper bound on fraction of margin errors, and
# a lower bound on the fraction of support vectors used. As you increase
# nu, the number of support vectors goes up, which overfits the training
# data and hurts generalization performance.
svmModel = svm(cls ~ ., data=trainingData, kernel="radial", type="nu-classification", nu=0.3)
summary(svmModel)
...
testData = sample_nonlinear_data(a, b, c, -2, 2, -2, 2, 50)
